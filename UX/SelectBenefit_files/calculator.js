(function ($) {
	...
	slideBox = function () {
	    ...
	    if (slideOrder === 3) {
	        ...
	        container.find('.c-blue-box-calculation span').html(_numberFormat(Math.round(((_data.disabilityCoverage / _data.salary) * 100) * 10) / 10));
	...
	coverageSlider = function (newValue) { 
	    var value = 0;
	    if (newValue != undefined && parseFloat(newValue) != NaN) {
	...
	mobileCoverageSlider = function (newValue) {
	    var value = 0;
	    if (newValue != undefined && parseFloat(newValue) != NaN) {
	        if (parseFloat(newValue) < 0) {
	            return;
	...
	showCalculationPopup = function (value) {
	    container.find('#calculatorContainer .c-popup').css('top', value.data('place')).stop().fadeTo(300, 1);
	},

	hideCalculationPopup = function (value) {
	    var popup = container.find('.c-popup');
	    popup.stop().fadeTo(300, 0, function () { popup.hide(); });

	},

    toggleCalc = true,
    togglePopupshowCalculationPopup = function () {
        if (toggleCalc) {
            showCalculationPopup($(this));
            toggleCalc = !toggleCalc;
        }
        else {
            hideCalculationPopup($(this));
            toggleCalc = !toggleCalc;
        }
	...
	mobilePrevSlide = function () {
	    ...
	    if (!$('.mobile-chart-box').is(':visible'))
	        $('.mobile-chart-box').show();
	},
	...
	init = function () {
	    ...
	    container
	        ...
	        .on('click', '.infoone, .infotwo ', togglePopupshowCalculationPopup)
    ...
    $(document).ready(function() {
        $(document).on('change', '#otherCoverage', updateSlider);

        function updateSlider() {
            var otherCoverage = $(this);
            if (otherCoverage.val() != NaN) {
                var newValue = parseFloat(otherCoverage.val());
                coverageSlider(newValue);
                mobileCoverageSlider(newValue);
